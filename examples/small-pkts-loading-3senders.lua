...
local SRC_IP2    = "70.1.1.2"
local DST_IP    = "10.1.1.2"
local SRC_PORT  = 36666
local DST_PORT  = 6666
local PKT_SIZE  = 60

function master(txPort1, txPort2, txPort3, rate)
    if not txPort1 or not txPort2 or not txPort3 or not rate then
        return print("usage: txPort1 txPort2 txPort3 rate(Gbps)")
    end
    txPort1 = txPort1 or 0
    txPort2 = txPort2 or 1
    txPort3 = txPort3 or 2
    rate = rate or 10
    ...
    stats.startStatsTask{txDevices = {txDev1, txDev2, txDev3}}
    ...
    mg.startTask("loadSlave", txDev3:getTxQueue(0), txDev3, rate, SRC_IP3)
    mg.waitForTasks()
end
